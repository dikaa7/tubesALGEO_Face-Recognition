...
def run(filename,met):
    ...
    # print('Query image ==========================================')
    if(met == 2):
        t1=Label(text="Cosine Similarity").place(x=10,y=20)
        names,vector = ma.match(filename, topn=3)
    elif(met == 1):
        t1=Label(text="Jarak Euclidean").place(x=10,y=20)
        names,vector = ma.matcheuclidean(filename, topn=3)
    # print('Result images ========================================')
    ...
    t1=Label(text='1. '+str(vector[1])).place(x=20,y=500)
    t1=Label(text='2. '+str(vector[2])).place(x=20,y=520)
    t1=Label(text='3. '+str(vector[3])).place(x=20,y=540)
    plt.show()
    return vector
...
class App(Frame):
    ...
    def __init__(self, master=None):
        ...
        self.met= IntVar()
        self.labeltext = "Face Recognition"
        fram = Frame(self)
        ...
        Button(fram, height=1,width=10, text='Open', command=self.open).place(x=30,y=400)
        Button(fram, text="Open File", command=self.open).pack(side=LEFT)
        Radiobutton(master, text='Euclidean distance', command = self.met1, value=1).place(x=20,y=450)
        Radiobutton(master, text='Cosine similarity', command=self.met2, value=2).place(x=220,y=450)
